...
public class ShowDetail {
	...
	//传入主键，自动【填充】好该类的其他属性
	public void generateClass(String goodsId,int userId) {
...
/*		OccasionDAO occasionDao = new OccasionDAO();
		...
		*/
		...
		if (shoesCollectionDAO.isCollection(shoe.getGoodsId(), userId)) {
			isCollection = "uncollection";//如果是已经收藏了的将其值设置成uncollection，此为页面的css类名，此处耦合度大，为不好的地方
		}else {
			isCollection = "collection";
		}
		if (shoesLikesDAO.isLiked(shoe.getGoodsId(), userId)) {
			isLiked = "unlike";
		}else {
			isLiked = "like";
		}
		//获取点赞数和收藏总数
		numsOfCollection = shoesCollectionDAO.findByProperty("goodsId", shoe.getGoodsId()).size();
		numsOfLike = shoesLikesDAO.findByProperty("goodsId", shoe.getGoodsId()).size();
