...
public class OnlineStoreDAO extends BaseHibernateDAO {
	...
	//鏍规嵁ID鏌ユ壘OnlineStore
	 public OnlineStore get(String url){
		...
		//通过品牌名brand找到该品牌的三个照片信息 showShoesImg
		
		public List<showShoesImg> getImgInfo(String brand) {
			log.debug("get image infomation of the brand");
			try {
				System.out.println("getImgIngo");
				String sql = "select img_url,ol.goods_id,brand,online_url "
				+ "from online_store as ol,shoes as sh where sh.brand=? and sh.goods_id=ol.goods_id ";
				 Query query = this.getSession().createSQLQuery(sql).setResultTransformer(Transformers.aliasToBean(showShoesImg.class));  
				 query.setString(0, brand);
				 query.setMaxResults(5);
				List<showShoesImg> showImgList = query.list();
				System.out.println("getImgIngo"+showImgList.size());
				 return showImgList;
			} catch (RuntimeException re ) {
				log.error("find image info by brand name failed", re);
				throw re;
			}
		}
